...
class SinglePlotter:
    ...
    def plot_forces(
        ...
        for idx, comp in enumerate(components):
            ...
            if enable_filter and filter_params:
                ...
                # Filtered data (solid)
                # FIXED: Extract filter_type separately
                ...
                try:
                    ...
                    print(f"Warning: Filter failed for {comp}: {e}")
                    # Just show original if filter fails
                    pass
            else:
                fig.add_trace(go.Scatter(
                    x=df['time'],
    ...
    def plot_moments(
        ...
        # Reuse forces plotting logic with different ylabel
        fig = self.plot_forces(df, components, title, enable_filter, filter_params, annotations)
        fig.update_layout(yaxis_title="Moment (N·m)")
        return fig
    ...
    def plot_subplots(
        ...
        filter_type = filter_params.get('filter_type', 'savgol') if filter_params else 'savgol'
        params_copy = {k: v for k, v in filter_params.items() if k != 'filter_type'} if filter_params else {}
        ...
        if layout == '2x2':
            ...
            if enable_filter and filter_params:
                try:
                    filtered_res = DataFilter.apply_filter(resultant, df['time'], filter_type, **params_copy)
                    fig.add_trace(
